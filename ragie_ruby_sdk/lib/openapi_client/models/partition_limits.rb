=begin
#Ragie API

#No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

The version of the OpenAPI document: 1.0.0

Generated by: https://openapi-generator.tech
Generator version: 7.14.0

=end

require 'date'
require 'time'

module OpenapiClient
  class PartitionLimits
    attr_accessor :pages_processed_limit_monthly

    attr_accessor :pages_hosted_limit_monthly

    attr_accessor :pages_processed_limit_max

    attr_accessor :pages_hosted_limit_max

    attr_accessor :video_processed_limit_monthly

    attr_accessor :video_processed_limit_max

    attr_accessor :audio_processed_limit_monthly

    attr_accessor :audio_processed_limit_max

    attr_accessor :media_streamed_limit_monthly

    attr_accessor :media_streamed_limit_max

    attr_accessor :media_hosted_limit_monthly

    attr_accessor :media_hosted_limit_max

    # Attribute mapping from ruby-style variable name to JSON key.
    def self.attribute_map
      {
        :'pages_processed_limit_monthly' => :'pages_processed_limit_monthly',
        :'pages_hosted_limit_monthly' => :'pages_hosted_limit_monthly',
        :'pages_processed_limit_max' => :'pages_processed_limit_max',
        :'pages_hosted_limit_max' => :'pages_hosted_limit_max',
        :'video_processed_limit_monthly' => :'video_processed_limit_monthly',
        :'video_processed_limit_max' => :'video_processed_limit_max',
        :'audio_processed_limit_monthly' => :'audio_processed_limit_monthly',
        :'audio_processed_limit_max' => :'audio_processed_limit_max',
        :'media_streamed_limit_monthly' => :'media_streamed_limit_monthly',
        :'media_streamed_limit_max' => :'media_streamed_limit_max',
        :'media_hosted_limit_monthly' => :'media_hosted_limit_monthly',
        :'media_hosted_limit_max' => :'media_hosted_limit_max'
      }
    end

    # Returns attribute mapping this model knows about
    def self.acceptable_attribute_map
      attribute_map
    end

    # Returns all the JSON keys this model knows about
    def self.acceptable_attributes
      acceptable_attribute_map.values
    end

    # Attribute type mapping.
    def self.openapi_types
      {
        :'pages_processed_limit_monthly' => :'Integer',
        :'pages_hosted_limit_monthly' => :'Integer',
        :'pages_processed_limit_max' => :'Integer',
        :'pages_hosted_limit_max' => :'Integer',
        :'video_processed_limit_monthly' => :'Integer',
        :'video_processed_limit_max' => :'Integer',
        :'audio_processed_limit_monthly' => :'Integer',
        :'audio_processed_limit_max' => :'Integer',
        :'media_streamed_limit_monthly' => :'Integer',
        :'media_streamed_limit_max' => :'Integer',
        :'media_hosted_limit_monthly' => :'Integer',
        :'media_hosted_limit_max' => :'Integer'
      }
    end

    # List of attributes with nullable: true
    def self.openapi_nullable
      Set.new([
        :'pages_processed_limit_monthly',
        :'pages_hosted_limit_monthly',
        :'pages_processed_limit_max',
        :'pages_hosted_limit_max',
        :'video_processed_limit_monthly',
        :'video_processed_limit_max',
        :'audio_processed_limit_monthly',
        :'audio_processed_limit_max',
        :'media_streamed_limit_monthly',
        :'media_streamed_limit_max',
        :'media_hosted_limit_monthly',
        :'media_hosted_limit_max'
      ])
    end

    # Initializes the object
    # @param [Hash] attributes Model attributes in the form of hash
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::PartitionLimits` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      acceptable_attribute_map = self.class.acceptable_attribute_map
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::PartitionLimits`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'pages_processed_limit_monthly')
        self.pages_processed_limit_monthly = attributes[:'pages_processed_limit_monthly']
      end

      if attributes.key?(:'pages_hosted_limit_monthly')
        self.pages_hosted_limit_monthly = attributes[:'pages_hosted_limit_monthly']
      end

      if attributes.key?(:'pages_processed_limit_max')
        self.pages_processed_limit_max = attributes[:'pages_processed_limit_max']
      end

      if attributes.key?(:'pages_hosted_limit_max')
        self.pages_hosted_limit_max = attributes[:'pages_hosted_limit_max']
      end

      if attributes.key?(:'video_processed_limit_monthly')
        self.video_processed_limit_monthly = attributes[:'video_processed_limit_monthly']
      end

      if attributes.key?(:'video_processed_limit_max')
        self.video_processed_limit_max = attributes[:'video_processed_limit_max']
      end

      if attributes.key?(:'audio_processed_limit_monthly')
        self.audio_processed_limit_monthly = attributes[:'audio_processed_limit_monthly']
      end

      if attributes.key?(:'audio_processed_limit_max')
        self.audio_processed_limit_max = attributes[:'audio_processed_limit_max']
      end

      if attributes.key?(:'media_streamed_limit_monthly')
        self.media_streamed_limit_monthly = attributes[:'media_streamed_limit_monthly']
      end

      if attributes.key?(:'media_streamed_limit_max')
        self.media_streamed_limit_max = attributes[:'media_streamed_limit_max']
      end

      if attributes.key?(:'media_hosted_limit_monthly')
        self.media_hosted_limit_monthly = attributes[:'media_hosted_limit_monthly']
      end

      if attributes.key?(:'media_hosted_limit_max')
        self.media_hosted_limit_max = attributes[:'media_hosted_limit_max']
      end
    end

    # Show invalid properties with the reasons. Usually used together with valid?
    # @return Array for valid properties with the reasons
    def list_invalid_properties
      warn '[DEPRECATED] the `list_invalid_properties` method is obsolete'
      invalid_properties = Array.new
      if !@pages_processed_limit_monthly.nil? && @pages_processed_limit_monthly < 0
        invalid_properties.push('invalid value for "pages_processed_limit_monthly", must be greater than or equal to 0.')
      end

      if !@pages_hosted_limit_monthly.nil? && @pages_hosted_limit_monthly < 0
        invalid_properties.push('invalid value for "pages_hosted_limit_monthly", must be greater than or equal to 0.')
      end

      if !@pages_processed_limit_max.nil? && @pages_processed_limit_max < 0
        invalid_properties.push('invalid value for "pages_processed_limit_max", must be greater than or equal to 0.')
      end

      if !@pages_hosted_limit_max.nil? && @pages_hosted_limit_max < 0
        invalid_properties.push('invalid value for "pages_hosted_limit_max", must be greater than or equal to 0.')
      end

      if !@video_processed_limit_monthly.nil? && @video_processed_limit_monthly < 0
        invalid_properties.push('invalid value for "video_processed_limit_monthly", must be greater than or equal to 0.')
      end

      if !@video_processed_limit_max.nil? && @video_processed_limit_max < 0
        invalid_properties.push('invalid value for "video_processed_limit_max", must be greater than or equal to 0.')
      end

      if !@audio_processed_limit_monthly.nil? && @audio_processed_limit_monthly < 0
        invalid_properties.push('invalid value for "audio_processed_limit_monthly", must be greater than or equal to 0.')
      end

      if !@audio_processed_limit_max.nil? && @audio_processed_limit_max < 0
        invalid_properties.push('invalid value for "audio_processed_limit_max", must be greater than or equal to 0.')
      end

      if !@media_streamed_limit_monthly.nil? && @media_streamed_limit_monthly < 0
        invalid_properties.push('invalid value for "media_streamed_limit_monthly", must be greater than or equal to 0.')
      end

      if !@media_streamed_limit_max.nil? && @media_streamed_limit_max < 0
        invalid_properties.push('invalid value for "media_streamed_limit_max", must be greater than or equal to 0.')
      end

      if !@media_hosted_limit_monthly.nil? && @media_hosted_limit_monthly < 0
        invalid_properties.push('invalid value for "media_hosted_limit_monthly", must be greater than or equal to 0.')
      end

      if !@media_hosted_limit_max.nil? && @media_hosted_limit_max < 0
        invalid_properties.push('invalid value for "media_hosted_limit_max", must be greater than or equal to 0.')
      end

      invalid_properties
    end

    # Check to see if the all the properties in the model are valid
    # @return true if the model is valid
    def valid?
      warn '[DEPRECATED] the `valid?` method is obsolete'
      return false if !@pages_processed_limit_monthly.nil? && @pages_processed_limit_monthly < 0
      return false if !@pages_hosted_limit_monthly.nil? && @pages_hosted_limit_monthly < 0
      return false if !@pages_processed_limit_max.nil? && @pages_processed_limit_max < 0
      return false if !@pages_hosted_limit_max.nil? && @pages_hosted_limit_max < 0
      return false if !@video_processed_limit_monthly.nil? && @video_processed_limit_monthly < 0
      return false if !@video_processed_limit_max.nil? && @video_processed_limit_max < 0
      return false if !@audio_processed_limit_monthly.nil? && @audio_processed_limit_monthly < 0
      return false if !@audio_processed_limit_max.nil? && @audio_processed_limit_max < 0
      return false if !@media_streamed_limit_monthly.nil? && @media_streamed_limit_monthly < 0
      return false if !@media_streamed_limit_max.nil? && @media_streamed_limit_max < 0
      return false if !@media_hosted_limit_monthly.nil? && @media_hosted_limit_monthly < 0
      return false if !@media_hosted_limit_max.nil? && @media_hosted_limit_max < 0
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] pages_processed_limit_monthly Value to be assigned
    def pages_processed_limit_monthly=(pages_processed_limit_monthly)
      if !pages_processed_limit_monthly.nil? && pages_processed_limit_monthly < 0
        fail ArgumentError, 'invalid value for "pages_processed_limit_monthly", must be greater than or equal to 0.'
      end

      @pages_processed_limit_monthly = pages_processed_limit_monthly
    end

    # Custom attribute writer method with validation
    # @param [Object] pages_hosted_limit_monthly Value to be assigned
    def pages_hosted_limit_monthly=(pages_hosted_limit_monthly)
      if !pages_hosted_limit_monthly.nil? && pages_hosted_limit_monthly < 0
        fail ArgumentError, 'invalid value for "pages_hosted_limit_monthly", must be greater than or equal to 0.'
      end

      @pages_hosted_limit_monthly = pages_hosted_limit_monthly
    end

    # Custom attribute writer method with validation
    # @param [Object] pages_processed_limit_max Value to be assigned
    def pages_processed_limit_max=(pages_processed_limit_max)
      if !pages_processed_limit_max.nil? && pages_processed_limit_max < 0
        fail ArgumentError, 'invalid value for "pages_processed_limit_max", must be greater than or equal to 0.'
      end

      @pages_processed_limit_max = pages_processed_limit_max
    end

    # Custom attribute writer method with validation
    # @param [Object] pages_hosted_limit_max Value to be assigned
    def pages_hosted_limit_max=(pages_hosted_limit_max)
      if !pages_hosted_limit_max.nil? && pages_hosted_limit_max < 0
        fail ArgumentError, 'invalid value for "pages_hosted_limit_max", must be greater than or equal to 0.'
      end

      @pages_hosted_limit_max = pages_hosted_limit_max
    end

    # Custom attribute writer method with validation
    # @param [Object] video_processed_limit_monthly Value to be assigned
    def video_processed_limit_monthly=(video_processed_limit_monthly)
      if !video_processed_limit_monthly.nil? && video_processed_limit_monthly < 0
        fail ArgumentError, 'invalid value for "video_processed_limit_monthly", must be greater than or equal to 0.'
      end

      @video_processed_limit_monthly = video_processed_limit_monthly
    end

    # Custom attribute writer method with validation
    # @param [Object] video_processed_limit_max Value to be assigned
    def video_processed_limit_max=(video_processed_limit_max)
      if !video_processed_limit_max.nil? && video_processed_limit_max < 0
        fail ArgumentError, 'invalid value for "video_processed_limit_max", must be greater than or equal to 0.'
      end

      @video_processed_limit_max = video_processed_limit_max
    end

    # Custom attribute writer method with validation
    # @param [Object] audio_processed_limit_monthly Value to be assigned
    def audio_processed_limit_monthly=(audio_processed_limit_monthly)
      if !audio_processed_limit_monthly.nil? && audio_processed_limit_monthly < 0
        fail ArgumentError, 'invalid value for "audio_processed_limit_monthly", must be greater than or equal to 0.'
      end

      @audio_processed_limit_monthly = audio_processed_limit_monthly
    end

    # Custom attribute writer method with validation
    # @param [Object] audio_processed_limit_max Value to be assigned
    def audio_processed_limit_max=(audio_processed_limit_max)
      if !audio_processed_limit_max.nil? && audio_processed_limit_max < 0
        fail ArgumentError, 'invalid value for "audio_processed_limit_max", must be greater than or equal to 0.'
      end

      @audio_processed_limit_max = audio_processed_limit_max
    end

    # Custom attribute writer method with validation
    # @param [Object] media_streamed_limit_monthly Value to be assigned
    def media_streamed_limit_monthly=(media_streamed_limit_monthly)
      if !media_streamed_limit_monthly.nil? && media_streamed_limit_monthly < 0
        fail ArgumentError, 'invalid value for "media_streamed_limit_monthly", must be greater than or equal to 0.'
      end

      @media_streamed_limit_monthly = media_streamed_limit_monthly
    end

    # Custom attribute writer method with validation
    # @param [Object] media_streamed_limit_max Value to be assigned
    def media_streamed_limit_max=(media_streamed_limit_max)
      if !media_streamed_limit_max.nil? && media_streamed_limit_max < 0
        fail ArgumentError, 'invalid value for "media_streamed_limit_max", must be greater than or equal to 0.'
      end

      @media_streamed_limit_max = media_streamed_limit_max
    end

    # Custom attribute writer method with validation
    # @param [Object] media_hosted_limit_monthly Value to be assigned
    def media_hosted_limit_monthly=(media_hosted_limit_monthly)
      if !media_hosted_limit_monthly.nil? && media_hosted_limit_monthly < 0
        fail ArgumentError, 'invalid value for "media_hosted_limit_monthly", must be greater than or equal to 0.'
      end

      @media_hosted_limit_monthly = media_hosted_limit_monthly
    end

    # Custom attribute writer method with validation
    # @param [Object] media_hosted_limit_max Value to be assigned
    def media_hosted_limit_max=(media_hosted_limit_max)
      if !media_hosted_limit_max.nil? && media_hosted_limit_max < 0
        fail ArgumentError, 'invalid value for "media_hosted_limit_max", must be greater than or equal to 0.'
      end

      @media_hosted_limit_max = media_hosted_limit_max
    end

    # Checks equality by comparing each attribute.
    # @param [Object] Object to be compared
    def ==(o)
      return true if self.equal?(o)
      self.class == o.class &&
          pages_processed_limit_monthly == o.pages_processed_limit_monthly &&
          pages_hosted_limit_monthly == o.pages_hosted_limit_monthly &&
          pages_processed_limit_max == o.pages_processed_limit_max &&
          pages_hosted_limit_max == o.pages_hosted_limit_max &&
          video_processed_limit_monthly == o.video_processed_limit_monthly &&
          video_processed_limit_max == o.video_processed_limit_max &&
          audio_processed_limit_monthly == o.audio_processed_limit_monthly &&
          audio_processed_limit_max == o.audio_processed_limit_max &&
          media_streamed_limit_monthly == o.media_streamed_limit_monthly &&
          media_streamed_limit_max == o.media_streamed_limit_max &&
          media_hosted_limit_monthly == o.media_hosted_limit_monthly &&
          media_hosted_limit_max == o.media_hosted_limit_max
    end

    # @see the `==` method
    # @param [Object] Object to be compared
    def eql?(o)
      self == o
    end

    # Calculates hash code according to all attributes.
    # @return [Integer] Hash code
    def hash
      [pages_processed_limit_monthly, pages_hosted_limit_monthly, pages_processed_limit_max, pages_hosted_limit_max, video_processed_limit_monthly, video_processed_limit_max, audio_processed_limit_monthly, audio_processed_limit_max, media_streamed_limit_monthly, media_streamed_limit_max, media_hosted_limit_monthly, media_hosted_limit_max].hash
    end

    # Builds the object from hash
    # @param [Hash] attributes Model attributes in the form of hash
    # @return [Object] Returns the model itself
    def self.build_from_hash(attributes)
      return nil unless attributes.is_a?(Hash)
      attributes = attributes.transform_keys(&:to_sym)
      transformed_hash = {}
      openapi_types.each_pair do |key, type|
        if attributes.key?(attribute_map[key]) && attributes[attribute_map[key]].nil?
          transformed_hash["#{key}"] = nil
        elsif type =~ /\AArray<(.*)>/i
          # check to ensure the input is an array given that the attribute
          # is documented as an array but the input is not
          if attributes[attribute_map[key]].is_a?(Array)
            transformed_hash["#{key}"] = attributes[attribute_map[key]].map { |v| _deserialize($1, v) }
          end
        elsif !attributes[attribute_map[key]].nil?
          transformed_hash["#{key}"] = _deserialize(type, attributes[attribute_map[key]])
        end
      end
      new(transformed_hash)
    end

    # Deserializes the data based on type
    # @param string type Data type
    # @param string value Value to be deserialized
    # @return [Object] Deserialized data
    def self._deserialize(type, value)
      case type.to_sym
      when :Time
        Time.parse(value)
      when :Date
        Date.parse(value)
      when :String
        value.to_s
      when :Integer
        value.to_i
      when :Float
        value.to_f
      when :Boolean
        if value.to_s =~ /\A(true|t|yes|y|1)\z/i
          true
        else
          false
        end
      when :Object
        # generic object (usually a Hash), return directly
        value
      when /\AArray<(?<inner_type>.+)>\z/
        inner_type = Regexp.last_match[:inner_type]
        value.map { |v| _deserialize(inner_type, v) }
      when /\AHash<(?<k_type>.+?), (?<v_type>.+)>\z/
        k_type = Regexp.last_match[:k_type]
        v_type = Regexp.last_match[:v_type]
        {}.tap do |hash|
          value.each do |k, v|
            hash[_deserialize(k_type, k)] = _deserialize(v_type, v)
          end
        end
      else # model
        # models (e.g. Pet) or oneOf
        klass = OpenapiClient.const_get(type)
        klass.respond_to?(:openapi_any_of) || klass.respond_to?(:openapi_one_of) ? klass.build(value) : klass.build_from_hash(value)
      end
    end

    # Returns the string representation of the object
    # @return [String] String presentation of the object
    def to_s
      to_hash.to_s
    end

    # to_body is an alias to to_hash (backward compatibility)
    # @return [Hash] Returns the object in the form of hash
    def to_body
      to_hash
    end

    # Returns the object in the form of hash
    # @return [Hash] Returns the object in the form of hash
    def to_hash
      hash = {}
      self.class.attribute_map.each_pair do |attr, param|
        value = self.send(attr)
        if value.nil?
          is_nullable = self.class.openapi_nullable.include?(attr)
          next if !is_nullable || (is_nullable && !instance_variable_defined?(:"@#{attr}"))
        end

        hash[param] = _to_hash(value)
      end
      hash
    end

    # Outputs non-array value in the form of hash
    # For object, use to_hash. Otherwise, just return the value
    # @param [Object] value Any valid value
    # @return [Hash] Returns the value in the form of hash
    def _to_hash(value)
      if value.is_a?(Array)
        value.compact.map { |v| _to_hash(v) }
      elsif value.is_a?(Hash)
        {}.tap do |hash|
          value.each { |k, v| hash[k] = _to_hash(v) }
        end
      elsif value.respond_to? :to_hash
        value.to_hash
      else
        value
      end
    end

  end

end
