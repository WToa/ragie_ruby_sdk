...
module RagieRubySdk
  class ConnectionLimitExceededWebhookPayload < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RagieRubySdk::ConnectionLimitExceededWebhookPayload` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RagieRubySdk::ConnectionLimitExceededWebhookPayload`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      limit_type_validator = EnumAttributeValidator.new('String', ["page_limit"])
    ...
    def limit_type=(limit_type)
      validator = EnumAttributeValidator.new('String', ["page_limit"])
    ...
    def hash
      [connection_id, partition, connection_metadata, limit_type].hash
    end
