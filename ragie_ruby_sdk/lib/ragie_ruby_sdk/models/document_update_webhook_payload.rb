...
module RagieRubySdk
  class DocumentUpdateWebhookPayload < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RagieRubySdk::DocumentUpdateWebhookPayload` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RagieRubySdk::DocumentUpdateWebhookPayload`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      status_validator = EnumAttributeValidator.new('String', ["ready", "failed", "refined", "indexed", "keyword_indexed", "paused_insufficient_credits"])
    ...
    def status=(status)
      validator = EnumAttributeValidator.new('String', ["ready", "failed", "refined", "indexed", "keyword_indexed", "paused_insufficient_credits"])
    ...
    def hash
      [document_id, status, partition, metadata, external_id, name, connection_id, sync_id, error].hash
    end
