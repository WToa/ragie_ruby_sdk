...
module RagieRubySdk
  class AuthenticatorList < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RagieRubySdk::AuthenticatorList` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RagieRubySdk::AuthenticatorList`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # Custom attribute writer method with validation
    # @param [Object] authenticators Value to be assigned
    def authenticators=(authenticators)
      if authenticators.nil?
        fail ArgumentError, 'authenticators cannot be nil'
      end
    ...
    def hash
      [pagination, authenticators].hash
    end
