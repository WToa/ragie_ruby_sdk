...
module RagieRubySdk
  class InstructionEntityExtractionLog < ApiModelBase
    attr_accessor :id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RagieRubySdk::InstructionEntityExtractionLog` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RagieRubySdk::InstructionEntityExtractionLog`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      status_validator = EnumAttributeValidator.new('String', ["extracted", "not_found", "error"])
      return false unless status_validator.valid?(@status)
      reason_code_validator = EnumAttributeValidator.new('String', ["NO_MATCH", "AMBIGUOUS", "OUT_OF_SCOPE", "SCHEMA_INVALID", "MODEL_ERROR"])
    ...
    def scope=(scope)
      validator = EnumAttributeValidator.new('String', ["document", "chunk"])
    ...
    def status=(status)
      validator = EnumAttributeValidator.new('String', ["extracted", "not_found", "error"])
    ...
    def reason_code=(reason_code)
      validator = EnumAttributeValidator.new('String', ["NO_MATCH", "AMBIGUOUS", "OUT_OF_SCOPE", "SCHEMA_INVALID", "MODEL_ERROR"])
    ...
    def hash
      [id, created_at, updated_at, instruction_id, document_id, scope, chunk_index, status, reason_code, reason, errors, page_count].hash
    end
