...
module RagieRubySdk
  class DurationLocation < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RagieRubySdk::DurationLocation` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RagieRubySdk::DurationLocation`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      location_type_validator = EnumAttributeValidator.new('String', ["duration"])
    ...
    def location_type=(location_type)
      validator = EnumAttributeValidator.new('String', ["duration"])
    ...
    def hash
      [location_type, start_time, end_time, duration].hash
    end
