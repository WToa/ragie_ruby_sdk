...
module RagieRubySdk
  class CreateInstructionParams < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RagieRubySdk::CreateInstructionParams` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RagieRubySdk::CreateInstructionParams`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # Custom attribute writer method with validation
    # @param [Object] entity_schema Value to be assigned
    def entity_schema=(entity_schema)
      if entity_schema.nil?
        fail ArgumentError, 'entity_schema cannot be nil'
      end
    ...
    def hash
      [name, active, scope, prompt, context_template, entity_schema, filter, partition].hash
    end
