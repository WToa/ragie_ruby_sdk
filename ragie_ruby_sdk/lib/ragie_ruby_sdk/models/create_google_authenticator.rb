...
module RagieRubySdk
  class CreateGoogleAuthenticator < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RagieRubySdk::CreateGoogleAuthenticator` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RagieRubySdk::CreateGoogleAuthenticator`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      provider_validator = EnumAttributeValidator.new('String', ["google"])
    ...
    def hash
      [provider, name, client_id, client_secret, domain, project_number].hash
    end
