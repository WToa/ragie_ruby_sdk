...
module RagieRubySdk
  class ConnectorSourceTypeInfo < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `RagieRubySdk::ConnectorSourceTypeInfo` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `RagieRubySdk::ConnectorSourceTypeInfo`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def source_type=(source_type)
      if source_type.nil?
        fail ArgumentError, 'source_type cannot be nil'
      end
    ...
    def hash
      [source_type, display_name, icon_url, docs_url].hash
    end
